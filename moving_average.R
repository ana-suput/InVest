...
moving_average <- function(tick, n, steps = 1){
  ...
  sma_data <- price_data
  sma_data[is.na(sma_data)==FALSE]<- NA
  diff_data <- sma_data
  warn_data <- sma_data[steps:nrow(sma_data),]
  
  ##Moving average
  for (i in 1:ncol(price_data)){
    sma_data[,i] <- SMA(price_data[,i], n)
    diff_data[,i] <- sign(price_data[,i] - sma_data[,i])
    
  }
  diff_data <- diff_data[(n):nrow(diff_data), ]
  obj$sma_data <- sma_data
